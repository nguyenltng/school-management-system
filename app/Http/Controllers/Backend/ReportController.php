...
class ReportController extends Controller
{
    ...
    public function studentMonthlyAttendance(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            $month = Carbon::createFromFormat('m/Y', $request->get('month'))->timezone(env('APP_TIMEZONE','Asia/Ho_Chi_Minh'));
    ...
    public function marksheetPublic(Request $request){
        ...
        if($request->isMethod('post')) {
            ...
            $publishedResult = DB::table('result_publish')
                ...
                ->whereDate('publish_date','<=', Carbon::now(env('APP_TIMEZONE','Asia/Ho_Chi_Minh'))->format('Y-m-d'))
                ->select('publish_date')
                ->first();
